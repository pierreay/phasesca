...
** PROG [2024-06-26 mer.] nRF52832 Clock
:PROPERTIES:
...
List of processings:
- =_extract_filtered= :: Apply a highpass filter on amplitude signal and lowpass
  filter on phase rotation. Results on amplitude is surprisingly good, with
  guessing the entire key with one bit of error (keyrank of 2^2). Results on
  phase rotation are better (2^100) but not good when using filtered profile
