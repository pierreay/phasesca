...
for signal_name in os.listdir("aes"):
  ...
  sig = np.load("aes/" + signal_name)
  if len(sig) == 16800:
    aes_signals.append(sig)

template = aes_signals[0][6000: -3300]
#plt.specgram(np.abs(template))
#plt.show()

'''
extracted_aes_signals = []
t = 0
for sig in aes_signals[1:]:
  
  #plt.plot(np.abs(sig))
  #plt.plot(butter_lowpass_filter(butter_lowpass_filter(np.abs(sig), cutoff=500e3, fs=8e6), cutoff=50e3, fs=8e6))
  n = 100
  samples = []
  factor = []
  for i in range(0, len(sig) - n, n):
    samples += [np.max(np.abs(sig[i:i+n])) for _ in range(n)]
    
  n = 200
  samples2 = []
  for i in range(0, len(samples) - n, n):
    samples2 += [np.mean(np.abs(samples[i:i+n])) for _ in range(n)]
    if np.mean(np.abs(samples[i:i+n]))<2200:
      extracted_aes_signals.append(sig[i:i+7000])#factor += [3000]*n if np.mean(np.abs(samples[i:i+n]))<2200 else [1500]*n
      break
    np.save("aes_extracted/" + str(t), sig[i:i+7000])
  t += 1
      #plt.show()
...
aes_signal_aligned = align_all(np.array(extracted_aes_signals), sr=8e6)
for aes_signal in aes_signal_aligned:
  plt.subplot(211)
  plt.specgram(np.abs(aes_signal))
  plt.subplot(212)
...
np.save("noaesmean", vals)
exit()
'''
aes_signals = np.array(aes_signals)
aes_signals2 = align_all(aes_signals, sr=8e6)

for aes_signal in aes_signals2:
  plt.plot(np.abs(aes_signal))
